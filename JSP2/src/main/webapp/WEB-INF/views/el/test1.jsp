...
<body>
  ...
  <h2>EL의 특징</h2>
  <pre>
  1. EL은 null을 "" = 빈칸으로 처리 하여
      + NullPointerException을 발생 시키지 않음
    
  2. get 이라는 단어를 거의 사용하지 않는다! 
    - EL은 표현(출력) 언어
     -> 값을 대입하고, 조건/반복하는 구문을 작성X
        오로지 출력! 
        (출력하려면 값을 얻어와야 되는게 당연하는거라 
        get 단어 사용 X)
  </pre>
...
<h3>EL은 자료형 변환 (Paesing)을 자동 수행 해준다</h3>

<ul>
<%-- EL은 Java로 변환 되지만 
문자열 비교는 == 비교연산자 사용 가능  --%>
  <li>${param.str == "abc"}</li>
<%-- HTMl에서 언덩온 데이터는 모두 String 
 하지만 EL 에서 연산 되는 자료형이 다를경우 
 자동으로 자료형 변환 (String ) wlsgodgownsek  --%>
  <li>${param.intNum == 100}</li>
  <li>${param.doubleNum == "3.14"}</li>
</ul>

<hr>
<h3> 같은 key같은 지닌 파라미터 얻어오기</h3>
